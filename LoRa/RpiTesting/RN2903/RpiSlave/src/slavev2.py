...
def on_message_callback(client, userdata, message):
    ...
    if message.payload.decode() == "done":
        add_new_data_to_csv(all_configs_file, distance, received_configs_array)
        successes = check_for_ones(received_configs_array)
        mqtt_send(mqttClient, "Subsite_Lora_Slave", str(successes))
        received_configs_array = create_array(all_configs_file)
    ...
    elif message.payload.decode() == "save":
        pass
...
def callback(message):
    ...
    if result[0] == "radio_err":
       ...
       # to the terminal
        ...
        received_configs_array[config_line] = 1
        # print(received_configs_array)
...
def add_new_data_to_csv(file, distance, array):
    """
...
def check_for_ones(array):
    ...
    for item in array:
        if item == 1:
            successful_configs.append(counter)
        counter += 1
